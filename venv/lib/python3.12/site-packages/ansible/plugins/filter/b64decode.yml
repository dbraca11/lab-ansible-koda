...
DOCUMENTATION:
  ...
  description:
    ...
    - Trying to store a binary blob in a string most likely corrupts the binary. To Base64 decode a binary blob, use the I(base64) command and pipe the encoded
      data through standard input. For example, in the M(ansible.builtin.shell) module, ``cmd="base64 --decode > myfile.bin" stdin="{{ encoded }}"``.
  positional: _input
  options:
    _input:
    ...
    urlsafe:
      description:
        - Decode string using URL- and filesystem-safe alphabet, which substitutes I(-) instead of I(+) and I(_) instead of I(/) in the Base64 alphabet.
      type: bool
      default: false
      required: false
      version_added: 2.19
